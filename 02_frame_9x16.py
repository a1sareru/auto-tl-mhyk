...
CROP_X1_RATIO = 943 / DEFAULT_WIDTH
...
SLIDE_X2_RATIO = 960 / DEFAULT_WIDTH
...
def extract_frames(video_path, debug, slides):
    ...
    with open(subtitle_path, "w") as sub_file:
        ...
        sub_file.write(f"PlayResX: {frame_width}\n")
        sub_file.write(f"PlayResY: {frame_height}\n\n")
        ...
        for start_time, end_time in high_similarity_intervals:
            start_str = f"0:{int(start_time / 60):02}:{start_time % 60:.2f}"
            end_str = f"0:{int(end_time / 60):02}:{end_time % 60:.2f}"
