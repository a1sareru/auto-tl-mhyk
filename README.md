...
`THRESHOLD_RATIO` 是一个介于 0 到 1 之间的浮点数，用于判断当前帧是否足够接近参考图像，从而触发“打轴”判断。
但它并不是一个绝对的相似度阈值，而是相对于视频中出现的最大相似度进行缩放的比值。

>例如，若某段视频中最高帧相似度为 0.987，且设定 `THRESHOLD_RATIO = 0.97`，则实际使用的判断阈值为 `0.987 × 0.97 ≈ 0.957`。

阈值越高，只有与参考图像几乎完全一致的帧才会触发识别；阈值越低，则更容易触发，但可能会误识别噪点。

>【理论上这段内容现在是正确的，实际操作中如果碰到问题请自行调整参数……开发者的智商在这种需要 reasoning 的问题面前实在不太够用】在某些情况下，例如参考图像正是从当前视频中截取时，可以适当调低 THRESHOLD_RATIO（例如 0.95），因为真实帧的相似度通常很高，不易误判。
